...
<nav id="menu" style="background:url(image/navBg.png repeat-x;)">
   ...
   <div id="whole">
    <section id="sec">
    <center><b><p style="font-family: Arial, Helvetica, sans-serif;font-size: 40px;color:red;">Party Details</p></b></center>
    	<div id="cont">
<%String clas=request.getParameter("clas");
ResultSet res=ob.viewparty(clas);
while(res.next()){%>
<div style="border: dotted 2px black;"><form method="post">
	<center><h1><%=res.getString("Name")%></h1></center>
	<center><img src="image/uploadFiles/<%=res.getString("Symbol")%>" height="100px" width="120px"></center>
	<center><h2>Leader:<%=res.getString("Leader") %></h2></center>
	<center><%=res.getString("Aboutus")%></center>
	<%String s=res.getString("Approve");%>
	<%if(s.equals("0")) {%>
	<center><input type="submit" id="submit" value="Add" formaction="reg.jsp?choice=8&Name=<%=res.getString("Name")%>&Leader=<%=res.getString("Leader")%>"></center>
	<%} %></form></div>
<%}
%>
    	</div>
