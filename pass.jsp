...
<div id="whole">
    <section id="sec">
    <center><b><p style="font-family: Arial, Helvetica, sans-serif;font-size: 40px;color:red;">Change Password</p></b></center>
    	<div id="cont">

    <form method="post" onSubmit="return validateForm()">
    <p class="contact">Old Password:</p>
    <input type="text" name="Password" placeholder="old password" required>
    <p class="contact">New Password:</p>
    <input type="password" name="pw" id="pw" placeholder="new password" required>
        <p class="contact">Confirm Password:</p>
    <input type="password" name="cpw" id="cpw" placeholder="confirm password" required>
    <div id="error" style="color:Red; font-size:12px;"></div><br>
    <input type="submit"  value="Change" class="buttom" formaction="pass.jsp?choice=4"/>
    </form>
</div>
<%
if(choice != null && choice.equals("4")){
	...
	boolean s=ob.changepw(name,Password,pw);
	if (s==true){
		%>
		...
		%>
		<script type="text/javascript">
		document.getElementById("error").innerHTML = "<u>Incorrect Old Password</u>";
		</script>
		<%
	}}
	 %>
